import { System, SafeMath, Protobuf, Arrays, authority, Token, error, StringBytes, u128 } from "@koinos/sdk-as";
...
export class Collections {
  ...
  get_owners(args: collections.get_owners_arguments): collections.get_owners_result {
    ...
    for (let index: u64= 1; index <= Constants.MAX_SUPPLY; index++) {
      let _token = this._state.getToken(index.toString());
      if(_token) {
  ...
  mint(args: collections.mint_arguments): collections.empty_object {
    ...
    const gallery = this._state.getGallery(to);
  ...
  transfer(args: collections.transfer_arguments): collections.empty_object {
    ...
    gallery_to.tokens.push(u128.fromString(token_id).toU64())
